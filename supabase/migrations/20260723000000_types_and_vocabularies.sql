...
-- Date:     2026-07-23
-- Revision: 4 (DP round-2 targeted corrections)
--
-- Contents:
...
--   Section D  — Content layer states                 (7 enums)
--   Section E  — Governance states and rules          (5 enums)
--   Section F  — Operational states and rules         (4 enums)
--   Section G  — Jurisdiction states                  (2 enums)
--   Section H  — Attribute layer structural types     (1 enum)
--
--   Section I  — Standard reference tables            (36 tables)
--   Section J  — Governed reference tables            (2 tables)
--   Section K  — Unit reference table                 (1 table)
--
--   Section L  — Privilege controls and row-level security
--
-- Total:  39 enum types · 39 reference/mapping tables
--
-- v4 Changes from v3:
--   REMOVED: escalation_resolution_type — DEFERRED (EscalationRecord not designed)
--   REMOVED: evidence_role enum — CONVERTED to evidence_roles reference table (I.36)
--   RENAMED: narrative_authenticity_status → narrative_attribution_status
--   REMOVED: cultural_governed from governance_lifecycle_condition (9→8 values)
--   REMOVED: IF NOT EXISTS / EXCEPTION WHEN duplicate_object (production migration must fail
--            on unexpected existing objects; idempotency belongs in reset/bootstrap scripts)
--   ADDED:   Explicit REVOKE ALL FROM anon / REVOKE DML FROM authenticated / GRANT SELECT
--   FIXED:   authority_context_policy_conditions UNIQUE NULLS NOT DISTINCT for NULL jurisdiction
--   REMOVED: 4 presumptive authority-context mapping seeds (contributor/historical contexts
--            are not subject lifecycle conditions)
--
-- Deferred (not in this migration):
--   escalation_record_status  — EscalationRecord not yet designed; state machine provisional
--   escalation_resolution_type — Co-deferred with escalation_record_status (EscalationRecord)
--   person_lifecycle_status   — Authority/derivation model unresolved; P2 principle applies
--   source_derivative_type    — Removed: duplicate of source_derivative_types ref table
--
-- Production migration semantics:
--   This migration is tracked and executed ONCE by the Supabase migration runner.
--   Schema DDL (CREATE TYPE, CREATE TABLE) has NO IF NOT EXISTS guards. If a type or
--   table already exists when this migration runs, it will fail — this is intentional.
--   Schema drift must be surfaced as a failure, not silently ignored.
--   Seed INSERT statements use ON CONFLICT DO NOTHING — seeds are idempotent by design.
--   For local dev/reset environments, use a separate bootstrap/reset script that drops
--   and recreates the schema before running this migration.
--
...
-- narrative_attribution_status: renamed from narrative_authenticity_status (v4).
-- Concerns narrator/author attribution, not content authenticity.
...
-- 8 values (v4: cultural_governed removed — cultural governance is not a person
-- lifecycle condition; represented as cultural_governance_required BOOLEAN on
-- ApprovalPolicy; see GOVERNANCE_MODELS.md §4.5).
...
-- ============================================================
-- SECTION F: Operational States and Behavioral Rules (4 enums)
-- Source: OPERATIONAL_MODELS.md §1–§2
--
-- NOTE: escalation_record_status AND escalation_resolution_type are DEFERRED.
-- EscalationRecord is not yet designed (OPERATIONAL_MODELS §4.1).
-- Both types belong to EscalationRecord; they must be co-designed.
-- escalation_default_action belongs to EscalationPolicy (designed) — retained here.
-- See DEFERRED_TYPE_REGISTER.md §1a and §1b.
-- ============================================================
...
-- Referenced by: authority_assignment_permitted_actions.action_type_code (junction table).
-- DECISION: Normalized to junction table authority_assignment_permitted_actions
-- (authority_assignment_id UUID FK, action_type_code TEXT FK, granted_at, granted_by, conditions).
-- This decision is recorded in SOURCE_TO_SCHEMA_TYPE_MATRIX.md Flag 1.
-- The junction table is created in the AuthorityAssignment migration, not here.
...
-- I.36 evidence_roles
-- Converted from evidence_role enum (DP correction, v4).
-- Semantic policy columns drive promotion rule queries in domain logic.
-- Referenced by: ClaimEvidence.evidence_role_code.
-- Protected view planned for client-facing access (see VOCABULARY_RLS_MATRIX.md).
CREATE TABLE evidence_roles (
    code                      TEXT        PRIMARY KEY,
    display_label             TEXT        NOT NULL,
    description               TEXT,
    may_raise_evidence_status BOOLEAN     NOT NULL DEFAULT FALSE,
    is_contradictory          BOOLEAN     NOT NULL DEFAULT FALSE,
    is_superseding            BOOLEAN     NOT NULL DEFAULT FALSE,
    is_active                 BOOLEAN     NOT NULL DEFAULT TRUE,
    deprecated_at             TIMESTAMPTZ,
    replaced_by_code          TEXT        REFERENCES evidence_roles(code),
    sort_order                INTEGER,
    created_at                TIMESTAMPTZ NOT NULL DEFAULT now()
);

INSERT INTO evidence_roles
    (code, display_label, description, may_raise_evidence_status, is_contradictory, is_superseding, sort_order)
VALUES
    ('supports',       'Supports',       'Source supports the claim; consistent with the assertion',               TRUE,  FALSE, FALSE, 10),
    ('corroborates',   'Corroborates',   'Source corroborates the claim from an independent line of evidence',    TRUE,  FALSE, FALSE, 20),
    ('contextualizes', 'Contextualizes', 'Source provides context but does not directly confirm or deny',          FALSE, FALSE, FALSE, 30),
    ('contradicts',    'Contradicts',    'Source directly contradicts the claim; does not supersede it',           FALSE, TRUE,  FALSE, 40),
    ('supersedes',     'Supersedes',     'Source replaces the claim with a newer or higher-authority assertion',   FALSE, FALSE, TRUE,  50)
ON CONFLICT (code) DO NOTHING;
...
CREATE TABLE authority_context_policy_conditions (
    ...
    notes                          TEXT,
    created_at                     TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT uq_auth_ctx_policy_conditions
        UNIQUE NULLS NOT DISTINCT (authority_context_code, governance_lifecycle_condition, jurisdiction_code)
);
...
ON CONFLICT ON CONSTRAINT uq_auth_ctx_policy_conditions DO NOTHING;
...
-- SECTION L: Privilege Controls and Row-Level Security
--
-- For every vocabulary table (39 total):
--   1. REVOKE ALL FROM anon              (override Supabase default grants)
--   2. REVOKE DML FROM authenticated     (explicit prohibition; not just absence of policy)
--   3. GRANT SELECT TO authenticated     (read-only access for authenticated sessions)
--   4. ALTER TABLE ENABLE ROW LEVEL SECURITY
--   5. CREATE POLICY vocab_read_authenticated (SELECT to authenticated; no WHERE filter —
--      is_active is a UI hint, not an access control gate; deprecated codes must be
--      readable for historical record display)
--
-- REVOKE/GRANT/ENABLE RLS: run in a DO loop (individually idempotent).
-- CREATE POLICY: listed individually outside the loop. These statements will fail
-- if the policy already exists — correct behaviour for a production migration that
-- the Supabase runner tracks and executes exactly once. Do not add IF NOT EXISTS.
--
-- No DML policy is created for authenticated — REVOKE covers this.
-- service_role bypasses RLS entirely; it manages vocabulary via migrations only.
-- See VOCABULARY_RLS_MATRIX.md for full privilege design rationale.
-- ============================================================

-- Step 1–4: REVOKE, GRANT, ENABLE RLS (DO loop — idempotent operations)
DO $$ DECLARE tbl TEXT;
BEGIN
  FOR tbl IN SELECT unnest(ARRAY[
    ...
    'storage_providers',
    'file_storage_roles',
    'artifact_source_relationships',
    'participant_roles',
    'event_types',
    'authority_basis_types',
    'conflict_resolution_purposes',
    'escalation_trigger_types',
    'contest_types',
    'jurisdiction_types',
    'person_name_usage_types',
    'source_derivative_types',
    'data_conflict_types',
    'erasure_regimes',
    'authority_context_policy_conditions',
    'claim_value_units',
    'organization_types',
    'creation_sources',
    'place_types',
    ...
    'authority_roles',
    'action_types',
    'person_name_derivation_methods',
    'pronoun_set_types',
    'evidence_roles'
  ])
  LOOP
    -- Override Supabase default grants: revoke all from anon, revoke DML from authenticated
    EXECUTE format('REVOKE ALL ON %I FROM anon', tbl);
    EXECUTE format('REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON %I FROM authenticated', tbl);
    -- Grant explicit SELECT to authenticated
...
-- Step 5: CREATE POLICY (individual statements — production migration, run once)
CREATE POLICY "vocab_read_authenticated"
  ON access_classifications AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON participation_roles AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON authority_contexts AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON link_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON submission_origins AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON narrative_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON content_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON source_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON artifact_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON storage_providers AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON file_storage_roles AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON artifact_source_relationships AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON participant_roles AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON event_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON authority_basis_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON conflict_resolution_purposes AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON escalation_trigger_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON contest_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON jurisdiction_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON person_name_usage_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON source_derivative_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON data_conflict_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON erasure_regimes AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON authority_context_policy_conditions AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON claim_value_units AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON organization_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON creation_sources AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON place_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON coordinate_precisions AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON vessel_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON community_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON series_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON creation_reasons AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON mention_roles AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON authority_roles AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON action_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON person_name_derivation_methods AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON pronoun_set_types AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);

CREATE POLICY "vocab_read_authenticated"
  ON evidence_roles AS PERMISSIVE FOR SELECT TO authenticated USING (TRUE);


-- ============================================================
-- END OF 0001_types_and_vocabularies.sql (v4)
-- Summary:
--   39 PostgreSQL enum types (↓2 from v3: escalation_resolution_type deferred,
--                              evidence_role converted to reference table)
--   39 reference/mapping tables (36 standard + 2 governed + 1 unit; ↑1 from v3:
--                                 evidence_roles added)
--   ~241 seed rows (↓12 from v3: 4 presumptive authority_context_policy_conditions
--                   seeds removed + escalation_resolution_type seeds removed)
--   Explicit REVOKE ALL FROM anon on all 39 tables
--   Explicit REVOKE DML FROM authenticated on all 39 tables
--   GRANT SELECT TO authenticated on all 39 tables
--   RLS enabled on all 39 tables with vocab_read_authenticated policy
--   0 IF NOT EXISTS guards on schema DDL
--   0 EXCEPTION WHEN duplicate_object suppressions
--   0 placeholder types
--   0 fabricated values
-- ============================================================
